...
function insertIframe(name, url){
    ...
    iframe.style.zIndex = '2147483500';
    iframe.style.display = 'block';
    iframe.id = "GoogleScholarAssistant";

    if (name != "black") 
	{
		iframe.name = "free#" + encodeURI(url);
	}
	else 
	{
		iframe.name = "pay#" + encodeURI(url);
	}
...
function run() {
    ...
    if (!doi)
	{
		console.log("we dont't have a doi");
        return;
    }
